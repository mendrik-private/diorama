...
- Manual zoom percentages are evaluated in device space (`zoom × rs`): the `1` shortcut always displays 100% and maps one source pixel to one device pixel, while the internal logical zoom is `1 / rs`. Hard zoom aligns values above 100% to whole device pixels but preserves manual percentages at or below 100%. Plus and minus step through 25%, 50%, 75%, and 100% in both filtering modes. Zoom < 100% and pinch states keep Nearest with a snapped origin.
...
- Drag outward from the centre; the press point stays at the centre as both radii grow. On release an `Annotation::Highlight` is created with the current colour and `seed` from a document counter mixed with the id. Effective crayon width scales continuously with the image's longest side as `max(2.8, max(width, height) * 8.4 / 1024)` native image pixels: 8.4 px at 1024, 16.8 px at 2048. The stored `style.width` remains canonical at 1 px for document compatibility and is ignored by rendering.
- Rendering: one continuous 480° hand-drawn oval, starting in the upper-left, with a final 120° overlapping arc across the top. Seeded harmonic wobble, a slight tilt, and inward drift separate the extra arc. The crayon has variable pressure, tapered ends, a softer ragged outer edge, and mottled pigment with paper flecks throughout its core. Grain scales with stroke width and uses rect-relative coordinates so moving and clipping preserve the texture. Rendering happens on a separate bounded layer before compositing, preserving underlying annotations and giving previews and exports the same appearance.
...
- Highlight: determinism per `(rect, seed)`; translation invariance; different seeds differ; one 480° sweep with a separated overlapping arc; continuous image-relative width scaling; stored width ignored; grain within the stroke core; texture preserved under translation, clipping, and compositing.
...
- Pencil: freehand/line/rectangle/circle commits are `Annotate(Create)` entries; each renders identically to the established Pencil rasterizer. A click-only freehand mark commits and remains unselected so it appears directly on the canvas without resize controls; dragged marks remain selected for editing. Secondary-button sampling cancels any active Pencil gesture before reading the colour and can never begin a stroke. Move/resize preserves pressure; undo/redo restores nodes.
