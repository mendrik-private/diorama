...
impl Field {
    ...
    fn convolve_vertical(&self, kernel: &[f64], cancel: &CancellationToken) -> Result<Self> {
    ...
    fn convolve_horizontal(&self, kernel: &[f64], cancel: &CancellationToken) -> Result<Self> {
        let radius = (kernel.len() / 2) as isize;
        let mut out = Self::new(self.w, self.h);
